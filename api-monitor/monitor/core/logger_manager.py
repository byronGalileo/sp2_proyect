...
from database.operations import LogOperations
from database.models import LogEntry, EventEntry, LogLevel, ServiceStatus as MongoServiceStatus

# Global log operations instance (will be initialized when needed)
log_operations = None
...
class LoggerManager:
    ...
    def __init__(self, log_path: str, log_level: str = "INFO", mongodb_config: Optional[Dict[str, Any]] = None):
        ...
        self.log_operations = None  # Instance-level log operations
    ...
    def _setup_mongodb(self):
        ...
        try:
            ...
            if "cluster" in self.mongodb_config:
                os.environ["MONGO_CLUSTER"] = self.mongodb_config["cluster"]

            # Initialize log operations AFTER setting environment variables
            self.log_operations = LogOperations()

            # Test MongoDB connection
            if self.log_operations.connection.connect():
    ...
    def log_service_status(
        ...
        # MongoDB logging
        if self.mongodb_enabled and self.log_operations:
            try:
                log_entry = LogEntry(
                ...
                self.log_operations.save_log(log_entry)
            except Exception as e:
    ...
    def log_remediation_attempt(
        ...
        if self.mongodb_enabled and self.log_operations:
            try:
                ...
                self.log_operations.save_event(event_entry)
